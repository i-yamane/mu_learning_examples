...
def bench_experiment1(args: Any, gen_data: Callable[[Any, bool], DataWithInfo]) -> Dict[str, Any]:
    ...
    data_tr = gen_data(args, train=False)  # type: ignore
    ...
    model_OracleXY = NN(
        ...
        record_loss=True,
        two_step=False,
        log_metric_label='OracleXY',
        device=device
    )
    time_start = time.time()
    model_OracleXY.fit_direct(data_tr.loader_xy_and_size[0])
    time_end = time.time()
    mlflow.log_metric('time_OracleXY', time_end - time_start)
    mse_OracleXY = mse_x2y_y(
        predict_y_from_x=model_OracleXY.predict_y_from_x,
        loader_xy=data_te.loader_xy_and_size[0],
        device=device)
    mlflow.log_metric('MSE_OracleXY', mse_OracleXY)
    res['MSE_OracleXY'] = mse_OracleXY
    print('MSE of OracleXY: {}'.format(mse_OracleXY))
    acc_OracleXY = acc_x2y_y(
        predict_y_from_x=model_OracleXY.predict_y_from_x,
        loader_xy=data_te.loader_xy_and_size[0],
        device=device)
    mlflow.log_metric('ACC_OracleXY', acc_OracleXY)
    res['ACC_OracleXY'] = acc_OracleXY
    print('ACC of OracleXY: {}'.format(acc_OracleXY))
    ...
    # time_end = time.time()
    # mlflow.log_metric('time_JointRR', time_end - time_start)
    ...
    # time_end = time.time()
    # mlflow.log_metric('time_JointBregMU', time_end - time_start)
    ...
    # time_end = time.time()
    # mlflow.log_metric('time_2StepRR', time_end - time_start)
    ...
    # time_end = time.time()
    # mlflow.log_metric('time_2StepBregMU_CE', time_end - time_start)
    ...
    # time_end = time.time()
    # mlflow.log_metric('time_Naive', time_end - time_start)
    # mse_Naive = mse_x2y_y(
